...
inline BoardPins detectBoardPins() {
    ...
    if (chip == "ESP32D0WDQ6") return board_TBeam();
    else if (chip == "ESP32-WROOM-32") return board_Heltec();
    else if (chip == "ESP32-PICO-D4") return board_TDeck();
    else return board_Default();
}
