...
const char* DiscordModule::name() { return "discord"; }
...
void DiscordModule::connectGateway() {
    ...
    ws.onEvent([this](WStype_t type, uint8_t *payload, size_t length) {
        if (type == WStype_TEXT) handleIncomingMessage(String((char*)payload));
    });
...
bool DiscordModule::send(const OpenRF_Packet &pkt) {
    ...
    // Real API POST goes here
    return true;
}

void DiscordModule::loop() { ws.loop(); }
